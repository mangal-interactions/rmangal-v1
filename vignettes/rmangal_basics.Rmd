...
It's possible to get a list of *all* taxa (future versions of the API will
implement search methods). This can be done with:

```{r listTaxa}
all_taxa <- listTaxa(conn)
length(all_taxa)
```

This object is an array of lists, and so the first taxa is accessed by
`all_taxa[1]`.
