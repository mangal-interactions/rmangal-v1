...
mangalPost <- function(api, type, data)
{
  ...
  if(is.null(api[[type]]) | !("post" %in% api[[type]]$verbs)) stop(stringr::str_c("This API do not permit POSTing objects of type ",type))
	...
	queryset <- httr::POST(stringr::str_c(api[[type]]$url, render_parameters(api$args)), body = jsonlite::toJSON(data, auto_unbox=TRUE), httr::add_headers("Content-type" = "application/json"))
